$(document).ready(function () {
  $(".select2").select2();
  const alert = function (alertHeading, alertText, alertIcon) {
    $.toast({
  ...
  let bulkUploadBtn = document.querySelector("#bulk_upload");
  ...
  uploadForm.addEventListener("submit", function (e) {
    e.preventDefault();
    let formInputData = new FormData(this);
    $.ajax({
      ...
      success: function (response) {
        ...
        if (response.message == "File uploaded successfully") {
          alert("Success!", "File uploaded successfully.", "info");
          setTimeout(() => {
            location = location.href;
          }, 2000);
        } else {
          alert("Failed!", `${response.message}`, "warning");
          ...
          render: function (data, type, row, meta) {
            if (type === "display") {
              if (row.admission_number) {
                let photo = "";
                if ((row.photo == "") & (row.gender == "Female")) {
                  photo = "undraw_profile_1.svg";
                } else if ((row.photo == "") & (row.gender == "Male")) {
          ...
          render: function (data, type, row, meta) {
            if (type === "display") {
              if (row.linked_status == "Linked") {
                data = `<span class="label label-success">Assigned</span>`;
              } else {
    ...
    if (clickedBtn.classList.contains("editBtn")) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          document.querySelector("#batch_id").text = response[0].batch_id;
          document.querySelector("#student_cat_id").value =
            response[0].student_category_id;
          document.querySelector("#blood_group").value =
            response[0].blood_group_id;
          document.querySelector("#blood_group").text =
            response[0].blood_group_id;
          document.querySelector("#student_type").value =
            response[0].student_type;
          document.querySelector("#student_type").text =
            response[0].student_type;
          ...
          document.querySelector("#nationalty").text =
            response[0].nationality_id;
          document.querySelector("#state").value =
          ...
          document.querySelector("#lgaOption").value =
            response[0].local_gov_of_origin;
          document.querySelector("#lgaOption").text =
            response[0].local_gov_of_origin;
